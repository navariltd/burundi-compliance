...
def get_stock_ledger_data(doc):
    ...
    if voucher_type == "Stock Reconciliation":
        movement_type, quantity_difference, movement_description = (
            get_voucher_doc_details(doc, voucher_type, voucher_no, item_code)
        )
    else:
        movement_type, movement_description = get_voucher_doc_details(
            doc, voucher_type, voucher_no, item_code
        )
...
def get_voucher_doc_details(
    stock_ledger_entry_doc, voucher_type, voucher_no, item_code
):
    """
    ...
    if voucher_type == "Stock Entry":
        ...
        movement_type = (
            get_item_movement_on_purchase_receipt_and_invoice_on_submit_and_cancel(
                stock_ledger_entry_doc, doc
            )
        )
        movement_description = get_stock_movement_description(doc)
        return movement_type, movement_description
        ...
        movement_type, movement_description = (
            get_item_movement_on_delivery_note_and_sale_invoice_on_submit_and_cancel(
                stock_ledger_entry_doc, doc
            )
        )
        return movement_type, movement_description
        ...
        movement_type, quantity_difference = get_stock_recon_movement_type(
            stock_ledger_entry_doc, doc, item_code
        )
        movement_description = get_stock_movement_description(doc)
        ...
        movement_type = get_item_movement_asset_capitalisation_on_submit_and_cancel(
            stock_ledger_entry_doc, doc
        )
        movement_description = get_stock_movement_description(doc)
        return movement_type, movement_description
...
def get_stock_movement_type(stock_ledger_entry_doc, doc):
    ...
    if (
        stock_ledger_entry_doc.actual_qty > 0
        and stock_entry_type in ["Material Receipt", "Manufacture"]
        or stock_ledger_entry_doc.actual_qty < 0
        ...
        return get_item_movement_on_repack_on_submit_and_cancel(
            stock_ledger_entry_doc, doc
        )
...
def get_stock_movement_description(doc):
    stock_movement_description = (
        doc.custom_stock_movement_description
        if doc.custom_stock_movement_description
        else ""
    )
...
def get_item_movement_on_delivery_note_and_sale_invoice_on_submit_and_cancel(
    stock_ledger_entry_doc, doc
):
    """
...
def get_item_movement_on_purchase_receipt_and_invoice_on_submit_and_cancel(
    stock_ledger_entry_doc, doc
):
    """
...
def get_item_movement_asset_capitalisation_on_submit_and_cancel(
    stock_ledger_entry_doc, doc
):
    """
