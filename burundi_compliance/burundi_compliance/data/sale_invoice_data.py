...
class InvoiceDataProcessor:
    ...
    def prepare_invoice_data(self):
        ...
        invoice_data = {
            ...
            "tp_type": (
                1
                if self.auth_details["type_of_taxpayer"] == "pour personne physique et"
                else 2
            ),
            ...
            "tp_trade_number": self.auth_details[
                "the_taxpayers_commercial_register_number"
            ],
            "tp_email": tp_email,
            "vat_taxpayer": (
                0
                if self.auth_details["subject_to_vat"] == "pour un non assujetti ou"
                else 1
            ),
            "ct_taxpayer": (
                0
                if self.auth_details["subject_to_consumption_tax"]
                == "pour un non assujetti ou"
                else 1
            ),
            "tl_taxpayer": (
                0
                if self.auth_details["subject_to_flatrate_withholding_tax"]
                == "pour un non assujetti ou"
                else 1
            ),
    ...
    def get_invoice_items(self):
        ...
        for item in self.doc.items:
            ...
            item_designation = item.description if item.description else item.item_name if item.batch_no else item.item_code
            items.append(
                {
    ...
    def get_sales_data_for_stock_update(self, method=None):
        ...
        for item in sales_return_items:
            ...
            obr_permission = item_doc.custom_allow_obr_to_track_stock_movement
            if not obr_permission:
                continue
    ...
    def confirm_tin_verified(self, doc):
        ...
        if customer.custom_gst_category == "Registered":
            if not customer.custom_tin_verified:
                frappe.throw(
                    f"Kindly go and verify the Tin number of this customer on <b>customer</b> doctype"
                )
