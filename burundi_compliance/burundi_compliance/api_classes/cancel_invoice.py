...
class InvoiceCanceller:
    def __init__(self, token: str):
        obr_base = OBRAPIBase()
        self.BASE_CANCEL_INVOICE_API_URL = obr_base.get_api_from_ebims_settings(
            "cancel_invoice"
        )
        self.token = token
    ...
    def _create_or_update_integration_request(self, response, data):
        ...
        if integration_req:
            try:
                integration_docs = frappe.get_all(
                    "Integration Request",
                    filters={
    ...
    def cancel_invoice(self, data) -> dict:
        try:
            response = requests.post(
                self.BASE_CANCEL_INVOICE_API_URL, json=data, headers=self._get_headers()
            ).json()
    ...
    def get_invoice(self, data):
        ...
        _invoices = frappe.get_all(
            "Sales Invoice",
            filters={"custom_invoice_identifier": invoice_identifier},
            fields=["name"],
        )
    ...
    def update_invoice(self, data):
        doc = self.get_invoice(data)
        frappe.db.set_value(
            "Sales Invoice", doc.name, "custom_ebms_invoice_cancelled", 1
        )
        frappe.db.commit()
