...
class TrackStockMovement:
    ...
    def __init__(self, token, max_retries=5):
        obr_base = OBRAPIBase()
        self.BASE_TRACK_STOCK_MOVEMENT_API_URL = obr_base.get_api_from_ebims_settings(
            "add_stock_movement"
        )
        self.token = token
    ...
    def _update_integration_request(
        self, response, stock_movement_data, doc, status="Completed"
    ):
        ...
        if integration_req:
            ...
            self.create_integration_request(
                stock_movement_data, str(response), "Null", doc, status=status
            )
    ...
    def post_stock_movement(self, stock_movement_data, doc):
        ...
        try:
            response = requests.post(
                self.BASE_TRACK_STOCK_MOVEMENT_API_URL,
    ...
    def check_if_integration_request_exist(self, doc):
        integration_request = frappe.db.exists(
            "Integration Request",
            {"reference_doctype": "Stock Ledger Entry", "reference_docname": doc.name},
        )
        return integration_request

    def create_integration_request(
        self, stock_movement_data, response, error_msg, doc, status="Failed"
    ):
        create_request_log(
            stock_movement_data,
