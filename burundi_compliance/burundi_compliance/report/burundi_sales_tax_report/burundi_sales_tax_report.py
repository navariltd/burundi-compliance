...
class KenyaSalesTaxReport(object):
    ...
    def get_sales_invoices(self):
        ...
        if company:
            sales_invoice_query = sales_invoice_query.where(
                sale_invoice_doc.company == company
            )
        if is_return == "Is Return":
            sales_invoice_query = sales_invoice_query.where(
                sale_invoice_doc.is_return == 1
            )
        if is_return == "Normal Sales Invoice":
            sales_invoice_query = sales_invoice_query.where(
                sale_invoice_doc.is_return == 0
            )
        if from_date:
            sales_invoice_query = sales_invoice_query.where(
                sale_invoice_doc.posting_date >= from_date
            )
        if to_date:
            sales_invoice_query = sales_invoice_query.where(
                sale_invoice_doc.posting_date <= to_date
            )
    ...
    def get_data(self):
        ...
        report_details = [
            ...
            if report_entry.get("taxable_value")
        ]

        for report_entry in report_details:
            if report_entry["pin_of_purchaser"]:
                self.registered_customers_total_sales += report_entry[
                    "invoice_total_sales"
                ]
                self.registered_customers_total_vat += report_entry["amount_of_vat"]
            else:
                self.unregistered_customers_total_sales += report_entry[
                    "invoice_total_sales"
                ]
