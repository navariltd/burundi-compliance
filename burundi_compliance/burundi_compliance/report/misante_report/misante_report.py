...
def execute(filters=None):
    ...
    earning_map, deduction_map = filter_salary_slip_details(
...
def get_earning_and_deduction_types():
    deduction_types = [
        "Assurance Misante Patronal",
        "Assurance Misante Employé",
        "MISANTE",
    ]  # Add the desired earning types
...
def get_salary_slips(filters, company_currency):
    SL = frappe.qb.DocType("Salary Slip")
    doc_status = {"Draft": 0, "Submitted": 1, "Cancelled": 2}

    query = frappe.qb.from_(SL).select(SL.star)

    if filters.get("docstatus"):
        query = query.where(SL.docstatus == doc_status[filters.get("docstatus")])

    if filters.get("from_date"):
        query = query.where(SL.start_date >= filters.get("from_date"))

    if filters.get("to_date"):
        query = query.where(SL.end_date <= filters.get("to_date"))

    if filters.get("company"):
        query = query.where(SL.company == filters.get("company"))

    if filters.get("employee"):
        query = query.where(SL.employee == filters.get("employee"))

    if filters.get("currency") and filters.get("currency") != company_currency:
        query = query.where(SL.currency == filters.get("currency"))
...
def get_salary_slip_details(
    salary_slips, currency, company_currency, earnings=None, deductions=None
):
    SL = frappe.qb.DocType("Salary Slip")
    ...
    if earnings:
        result = (
            frappe.qb.from_(SL)
            .join(SD)
            .on(SL.name == SD.parent)
            .where((SD.parent.isin(salary_slips)) & (SD.parentfield == earnings))
            .select(
                SD.parent,
                SD.salary_component,
                SD.amount,
        ...
        result = (
            ...
            .join(SD)
            .on(SL.name == SD.parent)
            .where((SD.parent.isin(salary_slips)) & (SD.parentfield == deductions))
            .select(
                SD.parent,
                SD.salary_component,
                SD.amount,
...
def filter_salary_slip_details(salary_slips, currency, company_currency):
    ...
    for ss_name, components in ss_ded_map.items():
        filtered_components = {
            ...
            if key
            in ["Assurance Misante Patronal", "Assurance Misante Employé", "MISANTE"]
        }
        if filtered_components:
