...
def retry_sales_invoice_post(invoice_data, doc):
    ...
    while retries < max_retries:
        try:
            ...
            frappe.log_error(
                _(f"OBR: Error during retry ({retries}/{max_retries})"),
                str(e),
                reference_doctype=doc.doctype,
                reference_name=doc.name,
            )
        ...
        try:
            ...
            if result.get("success") == True:
                ...
                if doc.doctype == "Stock Ledger Entry":
                    frappe.db.set_value(
                        doc.voucher_type, doc.voucher_no, "custom_etracker", 1
                    )
                return
            else:
...
def enqueue_stock_movement(data, doc):
    frappe.enqueue(
        "burundi_compliance.burundi_compliance.utils.background_jobs.retry_stock_movement",
        data=data,
        doc=doc,
        queue="short",
        timeout=10,
...
def retry_cancel_invoice(invoice_data, doc):
    ...
    while retries < max_retries:
        try:
            ...
            retries += 1
            frappe.log_error(
                f"Error during retry ({retries}/{max_retries}): {str(e)}",
...
def enqueue_cancel_invoice(invoice_data, doc):
    frappe.enqueue(
        "burundi_compliance.burundi_compliance.utils.background_jobs.retry_cancel_invoice",
        invoice_data=invoice_data,
        doc=doc,
        queue="short",
        timeout=10,
...
def retry_sending_invoice(invoice_identifier):
    ...
    if not doc.custom_einvoice_signatures:
        ...
        frappe.throw(
            "E-invoice already created for this invoice. Cannot retry sending invoice data to OBR."
        )
    doc.reload()
...
def get_stock_ledger_send_data(doctype, doc_name):
    ...
    for item in this_doc.items:
        stock_ledger_entry = frappe.get_all(
            "Stock Ledger Entry",
            filters={"item_code": item.item_code, "voucher_no": doc_name},
            fields=["*"],
        )
